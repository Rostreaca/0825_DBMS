...
-- 이승철 사원 또는 선승제 사원과 같은 부서인 사원들의 사원명 핸드폰 번호 조회
SELECT
       DEPT_CODE
  FROM
       EMPLOYEE
...
SELECT
       EMP_NAME
     , (SELECT DEPT_TITLE FROM DEPARTMENT WHERE E.DEPT_CODE =DEPT_ID) -- 성능저하됨(셀렉트 2번 호출)
  FROM
       EMPLOYEE E;
...
SELECT * FROM EMPLOYEE;
-- 전 직원들 중 급여가 가장 높은 상위 5명 줄세우기해서 조회

...
-- 소문자로 작성을해도 잘 동작을 하지만 대문자로 작성하는 습관을 들이는 것이 좋음
...
-- 4. EMPLOYEE테이블에서 EMP_NAME, EMAIL, PHONE 컬럼을 조회하세요.
SELECT 
       EMP_NAME
     , EMAIL
     , PHONE
...
-- EMPLYOEE테이블로부터 사원들의 사원명, 급여 조회 --> 급여가 300만원 이상인 사원들만
SELECT
       EMP_NAME
     , SALARY
  FROM
 ...
 * 
 */
-- EMPLOYEE 테이블에서 급여가 350이상 500이하인 사원들의 사번 이름 직급 코드(JOB_CODE)
SELECT 
       EMP_ID
     , EMP_NAME
     , JOB_CODE
...
SELECT EMP_ID || '번 사원' || EMP_NAME || '님의 핸드폰 번호는 ' || PHONE
       || '입니다.' AS "사원의 정보"
  FROM 
       EMPLOYEE;
 ...
 *  - NULLS LAST : 컬럼값이 NULL 일 경우 조회결과의 아래쪽에 배치 (오름차순일 경우 기본값)
 * 
 */

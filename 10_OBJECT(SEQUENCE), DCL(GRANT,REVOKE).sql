 ...
 *  ----------------- 생략가능
 *  CACHE 바이트크기 / NO CACHE -> 캐시메모리 쓸건지 말건지
 *                           -> CACHE_SIZE 기본값은 20 BYTE 
 * 
 *  * CACHE : 미리 발생할 값을 생성해서 저장을 해둘건지 말건지 여부지정
 *            값이 필요할 때마다 매 번 새롭게 값을 생성해내는것 보다는
 *            캐시공간에 미리 생성된 값을 가져다 쓰는것이 훨씬 속도적인 측면에서 이득이 있음
 ...
 *                 기존시퀀스 값에서 INCREMENT BY값만큼 증가한 값
...
-- CURRVAL은 마지막에 성공적으로 만들어진 NEXTVAL를 보여주는 임시값

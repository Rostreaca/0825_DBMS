...
-- 전체 사원들의 사번, 사원명, 직급코드, 직급명을 한 번에 조회
SELECT 
       EMP_ID
     , EMP_NAME
     , JOB_CODE
...
--> JOIN을 통해 연결고리에 해당하는 컬럼만 매칭시킨다면 마치 하나의 조회결과물처럼 조회 가능
 ...
 *       (LEFT OUTER)                 |           오른쪽 외부조인(RIHGT OUTER JOIN)
 ...
 * 
 * 테이블 간의 JOIN시 일치하지 않는 행도 포함시켜서 ResultSet 반환
...
-- 일치하는 칼럼이 존재하지 않더라도 오른쪽 테이블의 데이터는 무조건 다 조회
...
--> ORACLE (오라클에선 안됨) : 오라클 구문에서는 사용이 불가능!!
SELECT 
       EMP_NAME
     , DEPT_TITLE
  FROM
 ...
 * ( 두 쿼리문을 수행한 결과값을 무조건 더함. 합집하에서 중복 제거를 하지 않는 것)
 * => 중복행이 여러 번 조회될 수 있음
 */
...
SELECT
 ...
 WHERE
       SALARY > 3000000;
-- UNION 쓰려면 SELECT절에 기술하는 컬럼이 같아야 함

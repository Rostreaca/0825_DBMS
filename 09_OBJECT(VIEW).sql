...
-- 뷰의 장점 : 쿼리문이 엄청 긴게 필요할 때 그때그떄 작성하면 힘들다...
--          딱 한번 뷰로 만들어주면 그때부터는 뷰를 간단하게 조회할 수 있음.
 ...
 *     AS 서브쿼리;
 * 
 * 뷰 생성시 기존에 중복된 이름의 뷰가 존재한다면 갱신(변경)해주고
 * 없다면 새로운 뷰를 생성해줌
 * 
 */
...
SELECT
  ...
  FROM
	   EMPLOYEE;    
/*
 * 뷰 생성 시 SELECT절에 함수 또는 산술연산식이 기술되어있는 경우 뷰 생성이 불가능하기 때문에
 * 반드시 별칭을 지정해 주어야 함
 */
SELECT * FROM VW_EMP;

-- 별칭 부여 방법 두번쨰(모든 컬럼에다가 별칭을 지어주어야 함)
